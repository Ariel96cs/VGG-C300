...
def load_grayscale_vgg_model(input_size=256):
    ...
    for layer in model.layers[2:]:
        if "conv" in layer.name:
            vgg16_weights[str(input_size)+"_" + layer.name] = model.get_layer(layer.name).get_weights()

    del model


    # Custom build VGG16
    input = Input(shape=(input_size, input_size, 1), name='{}_input'.format(input_size))
    # Block 1
    x = Conv2D(64, (3, 3), activation='relu', padding='same', input_shape=(input_size, input_size, 1), data_format="channels_last", name='{}_block1_conv1'.format(input_size))(input)
    x = Conv2D(64, (3, 3), activation='relu', padding='same', name='{}_block1_conv2'.format(input_size))(x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='{}_block1_pool'.format(input_size))(x)

    # Block 2
    x = Conv2D(128, (3, 3), activation='relu', padding='same', name='{}_block2_conv1'.format(input_size))(x)
    x = Conv2D(128, (3, 3), activation='relu', padding='same', name='{}_block2_conv2'.format(input_size))(x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='{}_block2_pool'.format(input_size))(x)

    # Block 3
    x = Conv2D(256, (3, 3), activation='relu', padding='same', name='{}_block3_conv1'.format(input_size))(x)
    x = Conv2D(256, (3, 3), activation='relu', padding='same', name='{}_block3_conv2'.format(input_size))(x)
    x = Conv2D(256, (3, 3), activation='relu', padding='same', name='{}_block3_conv3'.format(input_size))(x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='{}_block3_pool'.format(input_size))(x)

    # Block 4
    x = Conv2D(512, (3, 3), activation='relu', padding='same', name='{}_block4_conv1'.format(input_size))(x)
    x = Conv2D(512, (3, 3), activation='relu', padding='same', name='{}_block4_conv2'.format(input_size))(x)
    x = Conv2D(512, (3, 3), activation='relu', padding='same', name='{}_block4_conv3'.format(input_size))(x)
    x = MaxPooling2D((2, 2), strides=(2, 2), name='{}_block4_pool'.format(input_size))(x)

    # Block 5
    x = Conv2D(512, (3, 3), activation='relu', padding='same', name='{}_block5_conv1'.format(input_size))(x)
    x = Conv2D(512, (3, 3), activation='relu', padding='same', name='{}_block5_conv2'.format(input_size))(x)
    x = Conv2D(512, (3, 3), activation='relu', padding='same', name='{}_block5_conv3'.format(input_size))(x)
    x = MaxPooling2D((8, 8), strides=(8, 8), name='{}_block5_pool'.format(input_size))(x)

    base_model = Model(inputs=input, outputs=x)

    base_model.get_layer('{}_block1_conv1'.format(input_size)).set_weights(new_block1_conv1)
    ...
    base_model.save('vgg_grayscale_{}.hdf5'.format(input_size))
    return base_model
